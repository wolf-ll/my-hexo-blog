...
<img src="DepGraph-Towards-Any-Structural-Pruning\image-20240605144536610.png" alt="image-20240605144536610" style="zoom:80%;" />
...
如图4 (c)所示，目标是**学习所有分组层之间的一致稀疏性，同时将某些维度归零为零。**作者将分组参数扁平化并合并为一个大的参数矩阵，其中检索所有属于第k个可调维数的参数，就像CNN块的第k个通道一样。现在，一致的稀疏性可以通过一个简单的加权收缩来促进（采用一个L2正则项，通过赋予参数组的不同正则权重 𝛾 来进行组稀疏化）
